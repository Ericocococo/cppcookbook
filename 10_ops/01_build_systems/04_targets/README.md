...
# 运行 Debug
./build-mingw-ninja-mc/Debug/app.exe

# 构建 Release
"$CMAKE" --build build-mingw-ninja-mc --config Release

# 运行 Release
./build-mingw-ninja-mc/Release/app.exe
```
...
| 文件 | 作用 |
|---|---|
| `app.vcxproj` | 主项目，对应 `add_executable(app ...)` |
...
用 VS 打开工程：双击 `app.slnx`（VS 2022 17.x+ 的新格式）或 `app.sln`，解决方案加载后三个项目均可见。
...
1. `File → Open` 选择 `04_targets/` 目录
2. CLion 自动识别 `CMakeLists.txt`（与 01_hello 流程相同）
3. 在 **Build 输出窗口**可验证 `-I`（include 路径）是否按预期传播
