...
# 运行
./build-mingw-ninja/find_package_demo.exe
```
...
# 运行
./build-mingw-make/find_package_demo.exe
```
...
# 运行 Debug
./build-mingw-ninja-mc/Debug/find_package_demo.exe

# 构建 Release
"$CMAKE" --build build-mingw-ninja-mc --config Release

# 运行 Release
./build-mingw-ninja-mc/Release/find_package_demo.exe
```
...
:: 运行
build-msvc-ninja\find_package_demo.exe
```
...
:: 运行
build-msvc-vs\Debug\find_package_demo.exe
```
...
:: 运行
build-msvc-vs\Debug\find_package_demo.exe
build-msvc-vs\Release\find_package_demo.exe
```
...
| 文件 | 作用 |
|---|---|
| `find_package_demo.vcxproj` | 主项目，对应 `add_executable(find_package_demo ...)` |
...
用 VS 打开工程：双击 `find_package_demo.slnx`（VS 2022 17.x+ 的新格式）或 `find_package_demo.sln`，解决方案加载后三个项目均可见。
...
1. `File → Open` 选择 `06_find_package/` 目录
2. CLion 自动配置
3. 在 **CMake 工具窗口**（View → Tool Windows → CMake）可查看完整配置日志，包括 `find_package` 的查找过程和结果
