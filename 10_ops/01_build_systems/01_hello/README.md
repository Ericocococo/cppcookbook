...
# 运行
./build-mingw-make/hello_cmake.exe
```
...
# 运行 Debug
./build-mingw-ninja-mc/Debug/hello_cmake.exe

# 构建 Release
"$CMAKE" --build build-mingw-ninja-mc --config Release

# 运行 Release
./build-mingw-ninja-mc/Release/hello_cmake.exe
```
...
:: 运行
build-msvc-vs\Debug\hello_cmake.exe
build-msvc-vs\Release\hello_cmake.exe
```
...
| 文件 | 作用 |
|---|---|
| `hello_cmake.vcxproj` | 主项目，对应 `add_executable(hello_cmake ...)` |
...
用 VS 打开工程：双击 `hello_cmake.slnx`（VS 2022 17.x+ 的新格式）或 `hello_cmake.sln`，解决方案加载后三个项目均可见。
