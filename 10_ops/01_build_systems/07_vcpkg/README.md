...
# 运行
./build-mingw-ninja/vcpkg_demo.exe
```

第一次 cmake 配置时，vcpkg 自动按 `vcpkg.json` 安装依赖，耗时取决于网络和编译速度。

> cmd 三步版：

```bat
:: 配置
D:/ProgramData/JetBrains/CLion20260101/bin/cmake/win/x64/bin/cmake.exe -B build-mingw-ninja -G Ninja -DCMAKE_CXX_COMPILER="D:/ProgramData/JetBrains/CLion20260101/bin/mingw/bin/g++.exe" -DCMAKE_MAKE_PROGRAM="D:/ProgramData/JetBrains/CLion20260101/bin/ninja/win/x64/ninja.exe" -DCMAKE_TOOLCHAIN_FILE=D:/software/vcpkg/scripts/buildsystems/vcpkg.cmake
...
# 运行
./build-mingw-make/vcpkg_demo.exe
```
...
```bat
:: 配置
D:/ProgramData/JetBrains/CLion20260101/bin/cmake/win/x64/bin/cmake.exe -B build-mingw-make -G "MinGW Makefiles" -DCMAKE_CXX_COMPILER="D:/ProgramData/JetBrains/CLion20260101/bin/mingw/bin/g++.exe" -DCMAKE_MAKE_PROGRAM="D:/ProgramData/JetBrains/CLion20260101/bin/mingw/bin/mingw32-make.exe" -DCMAKE_TOOLCHAIN_FILE=D:/software/vcpkg/scripts/buildsystems/vcpkg.cmake
...
# 运行 Debug
./build-mingw-ninja-mc/Debug/vcpkg_demo.exe

# 构建 Release
"$CMAKE" --build build-mingw-ninja-mc --config Release

# 运行 Release
./build-mingw-ninja-mc/Release/vcpkg_demo.exe
```
...
"%CMAKE%" -B build-msvc-vs -G "Visual Studio 18 2026" -A x64 -DCMAKE_TOOLCHAIN_FILE=D:\software\vcpkg\scripts\buildsystems\vcpkg.cmake

:: 构建（多配置，必须指定 --config）
"%CMAKE%" --build build-msvc-vs --config Debug

:: 运行
build-msvc-vs\Debug\vcpkg_demo.exe
```
...
```bat
:: 配置
"D:\Program Files\Microsoft Visual Studio\18\Community\Common7\IDE\CommonExtensions\Microsoft\CMake\CMake\bin\cmake.exe" -B build-msvc-vs -G "Visual Studio 18 2026" -A x64 -DCMAKE_TOOLCHAIN_FILE=D:\software\vcpkg\scripts\buildsystems\vcpkg.cmake
...
:: 运行
build-msvc-vs\Debug\vcpkg_demo.exe
build-msvc-vs\Release\vcpkg_demo.exe
```
...
| 文件                   | 作用                                                                        |
|----------------------|---------------------------------------------------------------------------|
| `vcpkg_demo.vcxproj` | 主项目，对应 `add_executable(vcpkg_demo ...)`                                   |
...
用 VS 打开工程：双击 `vcpkg_demo.slnx`（VS 2022 17.x+ 的新格式）或 `vcpkg_demo.sln`，解决方案加载后三个项目均可见。
...
1. `File → Open` 选择 `07_vcpkg/` 目录
2. CLion 自动识别 `CMakeLists.txt`，右下角点击 **加载**
3. **Settings → Build, Execution, Deployment → CMake**，在 **CMake options** 填入：

```
-DCMAKE_TOOLCHAIN_FILE=D:/software/vcpkg/scripts/buildsystems/vcpkg.cmake
```

4. 点击 **OK** 后 CLion 自动重新配置，vcpkg 开始安装依赖
5. 工具栏选择工具链（MinGW 或 Visual Studio）和构建类型
6. **构建** `Ctrl+F9`**运行** `Shift+F10`
