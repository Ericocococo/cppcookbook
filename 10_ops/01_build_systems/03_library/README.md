...
> **注意（动态库）**：运行 `use_shared.exe` 时，对应的 `.dll` 文件须与可执行文件位于同一目录或处于系统 `PATH` 中。Ninja 构建后
`.dll` 通常已输出到 `build-msvc-ninja\`，无需额外复制。
...
./build-linux/use_static
./build-linux/use_shared
```
...
`use_static` 和 `use_shared`，分别选中后点击运行按钮即可。
