...
# 运行
./build-mingw-make/use_static.exe
./build-mingw-make/use_shared.exe
```
...
# 运行 Debug
./build-mingw-ninja-mc/Debug/use_static.exe
./build-mingw-ninja-mc/Debug/use_shared.exe

# 构建 Release
"$CMAKE" --build build-mingw-ninja-mc --config Release

# 运行 Release
./build-mingw-ninja-mc/Release/use_static.exe
./build-mingw-ninja-mc/Release/use_shared.exe
```
...
:: 运行
build-msvc-ninja\use_static.exe
build-msvc-ninja\use_shared.exe
```
...
:: 运行
build-msvc-vs\Debug\use_static.exe
build-msvc-vs\Debug\use_shared.exe
```
...
build-msvc-vs\Debug\use_static.exe
build-msvc-vs\Debug\use_shared.exe
build-msvc-vs\Release\use_static.exe
build-msvc-vs\Release\use_shared.exe
```

> **注意（动态库）**：运行 `use_shared.exe` 时，对应的 `.dll` 文件须与可执行文件位于同一目录或处于系统 `PATH` 中。Ninja 构建后 `.dll` 通常已输出到 `build-msvc-ninja\`，无需额外复制。
...
**生成的 build 目录中 `.vcxproj` 的作用：**

| 文件 | 作用 |
|---|---|
| `use_static.vcxproj` | 主项目，对应 `add_executable(use_static ...)` |
| `use_shared.vcxproj` | 主项目，对应 `add_executable(use_shared ...)` |
...
用 VS 打开工程：双击 `cmake_library.slnx`（VS 2022 17.x+ 的新格式）或 `cmake_library.sln`，解决方案加载后所有项目均可见。
...
打开 `03_library` 目录，CLion 自动加载 `CMakeLists.txt`。工具栏 **Run/Debug Configurations** 下拉列表中会出现两个目标：`use_static` 和 `use_shared`，分别选中后点击运行按钮即可。
