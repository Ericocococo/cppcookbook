...
# 运行
./build-mingw-make/multi_sources.exe
```
...
# 运行 Debug
./build-mingw-ninja-mc/Debug/multi_sources.exe

# 构建 Release
"$CMAKE" --build build-mingw-ninja-mc --config Release

# 运行 Release
./build-mingw-ninja-mc/Release/multi_sources.exe
```
...
:: 运行
build-msvc-ninja\multi_sources.exe
```
...
:: 运行
build-msvc-vs\Debug\multi_sources.exe
```
...
:: 运行
build-msvc-vs\Debug\multi_sources.exe
build-msvc-vs\Release\multi_sources.exe
```
...
| 文件                      | 作用                                                                        |
|-------------------------|---------------------------------------------------------------------------|
| `multi_sources.vcxproj` | 主项目，对应 `add_executable(multi_sources ...)`                                |
...
用 VS 打开工程：双击 `multi_sources.slnx`（VS 2022 17.x+ 的新格式）或 `multi_sources.sln`，解决方案加载后三个项目均可见。
...
1. `File → Open` 选择 `02_sources/` 目录
2. CLion 自动加载（与 01_hello 流程相同），点击工具栏运行按钮即可
