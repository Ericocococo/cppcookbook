# 08_parquet_and_dll — 第八步：第三方依赖 + 拷贝 dll

用 arrow/parquet（vcpkg 安装）读真实 parquet 文件。本步对应框架 `build_engine.py` 的核心机制——**编译产物依赖第三方 dll，拷贝 .pyd 时必须连同 dll 一起拷贝**。
...
> pybind11 模块（.so）同样必须使用与 Python 相同的编译器。Linux 上的 Python 由 GCC 编译，因此 .so 用 GCC 构建即可。arrow/parquet 依赖通过系统包管理器或 vcpkg 安装，不需要手动拷贝 dll。

---

## 5. CLion IDE

1. `File → Open` 选择 `08_parquet_and_dll/` 目录
2. CLion 自动识别 `CMakeLists.txt`，右下角点击**加载**
3. CMake Options 中加 `-DCMAKE_TOOLCHAIN_FILE=D:/software/vcpkg/scripts/buildsystems/vcpkg.cmake -Dpybind11_DIR=... -DPython_EXECUTABLE=...`
4. **构建** `Ctrl+F9`
...
.pyd 是 Python 的动态库，但它内部链接了 `arrow.dll`、`parquet.dll` 等。Windows 加载 .pyd 时会去**同目录**找这些 dll，找不到就 import 失败。MSBuild 编译时把依赖 dll 部署到了 build_py/Release/ 下，所以拷贝时必须一起拷走。
