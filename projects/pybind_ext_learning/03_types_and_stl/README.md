# 03_types_and_stl — 第三步：多种类型 + STL 容器

上一步只传了 int，本步学会传 double / string / vector / map，以及 `py::arg` 关键字参数。
...
| `CMakeLists.txt` | 和第二步一样的 pybind11 工程 |
| `test_types.py` | 验证脚本 |
...
1. `File → Open` 选择 `03_types_and_stl/` 目录
