# 02_one_function — 第二步：第一个 pybind11 模块
...
> 本步故意不用 build.py，让你亲手跑一遍 cmake 命令，理解编译流程。第五步才引入自动化。
...
1. `File → Open` 选择 `02_one_function/` 目录
...
print(hello.__doc__)       # → "第一步：最小 pybind11 模块"
```
