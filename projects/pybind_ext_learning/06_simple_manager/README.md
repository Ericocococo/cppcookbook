# 06_simple_manager — 第六步：单个数据管理器

从"绑定一个类"到"绑定一个完整的数据管理器"。本步只有一个 Mgr（CFHSGMgr），用最简单的 `unordered_map + 线性扫描`。下一步再加二分查找和 cur_ns 防未来函数。
...
```bash
D:\ProgramData\anaconda3\envs\quant311\python build.py --dest lib && D:\ProgramData\anaconda3\envs\quant311\python test_fhsg.py
```
...
1. `File → Open` 选择 `06_simple_manager/` 目录
