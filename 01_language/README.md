...
| 目录           | 内容                                              | 状态 |
|--------------|-------------------------------------------------|------|
| [01_basics](01_basics/README.md)    | 内置类型、值类别、引用/指针、控制流、函数、命名空间                      | ✅ 入门主线（13 目录全建成） |
| [02_oop](02_oop/README.md)       | 类深入、构造/RAII、继承、多态、运算符重载、Rule of 0/3/5               | ✅ 深化专章（6 目录全建成） |
| [03_templates](03_templates/) | 函数模板、类模板、全/偏特化、变参模板、SFINAE                      | ⏳ 深化专章（待填充） |
| [04_modern](04_modern/README.md)    | 补充章：lambda 补充、移动语义、结构化绑定、Concepts、Modules       | 🚧 建设中（仅 01_lambda 建成） |
| [05_memory](05_memory/)    | 智能指针、内存布局、自定义分配器、内存模型、placement new             | ⏳ 深化专章（待填充） |
| [06_expert](06_expert/)    | type_traits、constexpr/consteval、静态反射、UB 与陷阱、ABI | ⏳ 深化专章（待填充） |
...
> `01_basics/08_oop`（入门）和 `02_oop`（深化）不是重复，是"先会用、再精通"两个层次：02_oop 里 basics 讲过的内容只标"回顾"，不再展开。
> 当前进度：01_basics ✅、02_oop ✅ 已建成；其余 ⏳/🚧 章待填充（先完成 01_basics 再进深化章的顺序不变）。
