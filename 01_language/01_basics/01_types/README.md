...
指针存的是**内存地址**：64 位系统的地址固定 8 字节，所以**所有指针都一样大，与指向什么类型无关**（`int*`、`double*`、`void*` 都是 8）。这就是上面 `foo` 里只有 8 的原因——传进函数的只是数组首元素的地址，数组本体没跟进来。

```cpp
std::cout << sizeof(int*)    << "\n";  // 8（64 位系统）
std::cout << sizeof(double*) << "\n";  // 8（和指向类型无关！）
std::cout << sizeof(void*)   << "\n";  // 8
// 32 位系统上则都是 4——地址位宽决定指针大小
```

> 指针是什么、为什么地址能当参数传，在 06_pointers_refs 章系统讲解。这里只需记住结论：指针大小由平台地址位宽决定（64 位 = 8），与类型无关；数组传进函数后只剩指针，长度必须另传。
...
`size_t` 是标准库定义的**无符号整数类型别名**，专门表示大小和索引。`sizeof` 运算符的返回类型就是 `size_t`。

**"无符号整数"是一整族类型**（`unsigned char` 1 字节 / `unsigned short` 2 字节 / `unsigned int` 4 字节 / `unsigned long long` 8 字节）——`size_t` 不是 `unsigned char`（它太小，最大才 255，装不下数组长度），而是标准库替你选好的"**足够大**"的那个：64 位系统上是 8 字节的 `unsigned long long`。标准库保证：任何对象的大小、任何数组的长度，`size_t` 都装得下。所以不用记它具体对应哪个类型，看到 `size_t` 就知道"这里需要一个大小/长度"。

注意它是无符号的，和负数比较或做减法时可能产生意外结果：
