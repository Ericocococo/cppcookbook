...
反斜杠 `\` 开头的字符序列是转义字符，用于表示无法直接输入的特殊字符。单引号 `'` 包裹的是 `char` 类型，双引号 `"` 包裹的是字符串字面量。
...
std::cout << "反斜杠\\\n";     // \\  输出一个反斜杠
std::cout << "引号\"里\"\n";    // \"  输出双引号
// \0  空字符（字符串结束标志，C 风格字符串的末尾隐含一个 \0）
```
