...
用空的大括号 `{}` 初始化数组，所有元素被置零。没有 `{}` 的话，局部数组的值是不确定的。
...
原始数组作为函数参数时退化为指针，丢失长度信息。函数内 `sizeof(arr)` 只有指针大小（8 字节），不是数组大小。解决方案：用 `std::array`，或额外传一个 `size` 参数。

```cpp
void f(int arr[]) {
    // sizeof(arr) 只有指针大小，不是数组大小！
    // 等价于 void f(int* arr)
}
