...
// 知识点 2.1：std::array（C++11）：固定大小，安全
...
void demo03_vector()
{
    ...
    // 知识点 3.2：push_back：可能触发扩容（重新分配，通常容量翻倍）
    ...
    // 知识点 3.5：emplace_back：直接在末尾构造，比 push_back 少一次复制

    v.emplace_back(99);
    std::cout << "  emplace_back(99): back=" << v.back() << "\n";

    // 知识点 3.6：insert / erase
...
// 知识点 5.1：std::string_view（C++17）：只读视图，不复制

void printSV(std::string_view sv)
{
    // 知识点 5.2：string_view：只读引用，不分配内存，比传 const string& 更灵活
...
void demo05_string_view()
{
    ...
    // 知识点 5.3：string_view 的子串也不复制

    std::string_view sv = s;
    std::cout << "  sv.substr(0,5)=\"" << sv.substr(0, 5) << "\"（不分配内存）\n";

    // 知识点 5.4：注意：string_view 不拥有数据，原字符串销毁后 string_view 成悬空引用
