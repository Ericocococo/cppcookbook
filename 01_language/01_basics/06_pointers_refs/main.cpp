...
void demo01_reference()
{
    ...
    // 知识点 1.3：const 引用：可绑定临时值

    const int& cref = 42;
    std::cout << "  const int& cref=42: " << cref << "\n";

    // 知识点 1.4：右值引用（C++11）：绑定临时值，用于移动语义
...
void demo03_const_pointer()
{
    ...
    // 知识点 3.2：const int* p：指向 const 的指针——不能改值，能改指向
    ...
    // 知识点 3.3：int* const p：const 指针——能改值，不能改指向
    ...
    *p2 = 99; // 可以
    ...
    // 知识点 3.4：const int* const p：都不能改
...
void demo04_pointer_arithmetic()
{
    ...
    // 知识点 4.4：注意：指针算术只在同一数组内有意义，越界是未定义行为
}

// 知识点 5.1：void* 指针：万能指针
...
// 知识点 7.1：智能指针（C++11）：自动释放内存

void demo07_smart_pointers()
{
    std::cout << "\n⑦ 智能指针\n";

    // 知识点 7.2：unique_ptr：独占所有权，离开作用域自动释放
    ...
    // 知识点 7.4：weak_ptr：弱引用，不增加引用计数，用于解决循环引用
